...
def face_exctraction(frame, background):
    res = frame
    kernel = np.ones((5,5),np.uint8)
    res = cv2.bitwise_xor(background,cv2.bitwise_not(frame), res)
...
def color_peau(frame):
    rects = detector(frame, 0)
    for rect in rects:
        shape = predictor(frame, rect)
        shape = face_utils.shape_to_np(shape)

    pos = [shape[35], shape[14]]
    centre = [int(pos[0][0] + pos[1][0])/2, int(pos[0][1] + pos[1][1])/2]
    col = frame[centre[0], centre[1]]
    col1 = [col[0],col[1],col[2]]

    pos = [shape[2], shape[31]]
    centre = [int(pos[0][0] + pos[1][0])/2, int(pos[0][1] + pos[1][1])/2]
    col = frame[centre[0], centre[1]]
    col2 = [col[0],col[1],col[2]]

    noir = dist2vect([0,0,0], col)
    bronze = dist2vect([176,85,10], col)
    blanc = dist2vect([176,165,110], col)
...
image = cv2.imread('hqdefault.jpg', 1)
copie = cv2.imread('copie.jpg', 1)

face_exctraction(image, copie)
